...
Claude is reserved for explicitly assigned high-value coding and deep design work, such as a focused TapDAT architecture review. Do not run recurring monitoring, broad connector sweeps, routine COC orchestration, or multi-destination status publication. Codex and deterministic connector infrastructure own COC intake, deduplication, reconciliation, calculated aging, connector-health tracking, and action-candidate routing. The retired monitor's Notion and GitHub outputs are historical provenance only; do not execute `ACS-ACT-002` unless it is reconciled against the canonical architecture.
