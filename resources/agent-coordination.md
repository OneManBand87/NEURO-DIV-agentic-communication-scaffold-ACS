...
The [private ACS Command Center](https://acs-command-center.onemanband87.chatgpt.site) is the ADHD-friendly management surface for current priorities, universal intake, exact-payload approvals, recruiter response SLAs, the six approved parent branches, project plans and timeline health, agent status, suggested next actions, and AI usage controls. Version 0.3 is the current owner-only production deployment and includes universal intake with verified native macOS routing.

The Central Command Center (CCS), implemented by the ACS Command Center, is the mandatory orchestration and visibility layer for all NEURO-DIV work in which Codex participates. Codex-initiated or Codex-assisted requests, actions, device-control paths, approvals, state changes, blockers, outputs, and completion evidence must be created, routed, or mirrored through CCS. ChatGPT Remote, iPhone Mirroring, Shortcuts, mobile device management, connectors, and local tools are subordinate execution transports beneath CCS and must not be represented as alternative top-level workflows. If a required CCS capability is not yet implemented, Codex must record the need and current state in CCS and preserve CCS visibility rather than silently bypassing it. This standing rule was recorded in production intake item `intake-c823b88f-982d-4ee6-abd9-e476b9f84d8f` and read back from canonical brief revision `ALtnJHzSSCzYPOt6ef5Qk_sbUmYApd8rAvhsC82uP8J72gxVuN7HYWenZuIMI73L73KC4ybHc0arNn3N5Nq5n7PE0D6AOSs0VAmbI-leIg`.
...
Universal-intake v0.3 adds one normalized queue for screenshots, screen recordings, files, URLs, selected text, and media. The macOS screenshot destination and event-driven LaunchAgent are installed; `Send to NEURO-DIV` and `Process NEURO-DIV Intake` Shortcuts are configured; device secrets remain in Keychain; metadata is bounded and deduplicated; raw inputs remain in device-synced Apple intake storage; and routed artifacts remain canonical in Google Drive. Production write/read-back, a real screenshot, and a real Shortcut input are verified end to end. Physical-iOS Share Sheet synchronization remains unverified.
