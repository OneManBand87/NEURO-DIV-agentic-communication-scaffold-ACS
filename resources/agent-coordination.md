...
Siri is secretary-only for spoken-word intake. The iPhone Action Button is the preferred primary trigger and should run the dedicated open-line recording Shortcut directly; Siri is optional fallback only. Action Button activation starts or opens the recorder but does not authorize transcription, interpretation, submission, or downstream action. Siri must not function as the conversational operator, interpret the thought, infer completion from silence, interrupt, clarify, suggest, summarize, conclude, or submit on the user's behalf. The required capture primitive is an open-line recording that continues through long pauses, topic changes, self-correction, repetition, and nonlinear ADHD speech until the user explicitly stops it through the recorder control, with pause/resume support. Preserve the original audio as the primary source; any transcript is a linked derivative aid that may contain errors. Only after explicit completion may CCS transcribe, interpret, classify, or route the recording, and all derived actions remain subject to normal verification and approval controls. If a Siri or Shortcut path cannot guarantee explicit-stop recording and original-audio preservation, use a button, recorder control, or another open-line capture surface instead.
