...
- Direct dictation through macOS or iPhone Voice Control or system Dictation into the COC Quick Capture box or another NEURO-DIV text-entry surface is permitted for fast, ordinary prompting.
...
- Listening may remain hands free, but direct text enters COC only through an explicit user submission command or action. Silence, pauses, and topic changes never submit automatically.
...
- After explicit completion, the recording may enter COC intake for downstream transcription and interpretation. Directly dictated text may enter COC after explicit submission. Any proposed task, decision, state change, or external action from either path remains subject to normal COC verification and approval controls.
...
- After capture, `Transcribe Audio` may produce a complete transcript. `Use Model` should use Apple's `On-Device` model by default for this simple, privacy-sensitive clerical pass. Private Cloud Compute or COC processing requires a demonstrated need; the ChatGPT extension model remains excluded from this route.
- The interpretation output is a strict JSON sidecar named `NEURO-DIV Voice Intake - [six-digit capture ID].coc-intake.json` beside the matching audio file.
...
- A proposed action is never an approval, commitment, calendar event, message, or completed task. Material downstream action remains subject to normal COC verification and approval controls.
...
- The Mac router validates the sidecar, pairs it with the matching audio, uses the guarded semantic title for the single COC intake item, retains the original audio filename, and carries the structured JSON as captured derivative context. Invalid or absent sidecars leave the raw-audio fallback route intact.
...
- Final implementation synchronization was accepted and read back through production COC intake item `intake-e6236b53-127d-4649-9a5c-044eba855451`.
...
- Apply the contingent routing protocol before every proposed Apple Intelligence use. The permitted routes are `deterministic`, `apple-on-device`, `coc-reasoning`, and `defer-for-review`; every route remains visible to COC and none may execute an action.
- Use `deterministic` for routine COC status briefs, exact record operations, reminders, and device control when current structured records can produce the result without interpretation. Do not invoke a model merely to make routine status sound conversational.
...
- Use `coc-reasoning` whenever the task needs cross-record context, current external information, connectors or tools, consequential judgment, or any action-producing decision. Also use COC when the on-device result fails schema/title validation or has low confidence.
...
- Privacy preference is on-device processing for simple sensitive transformations, but sensitivity never authorizes Apple Intelligence to bypass COC, normal approval controls, or preserved-source verification.
- Siri, Alexa, Siri-to-ChatGPT, and the ChatGPT Shortcut model are prohibited fallback routes. A failure must return to COC or review, never degrade into an assistant conversation.
- Alexa is outside the NEURO-DIV and COC ecosystem. Independent use for music playback or optional room-light control is not an integration and creates no COC dependency; no Alexa endpoint, skill, monitor, or future workflow is planned.
...
- Production COC intake item `intake-7267ed62-1160-41b5-b65b-d48ef2881707` mirrors the contingent-routing protocol, Alexa exclusion, test evidence, canonical revision, and remaining iPhone Mirroring boundary; it was read back from the production state API.
...
- The Siri-to-ChatGPT experience is excluded and may not serve as a COC front door, spoken intake surface, conversational operator, or task executor.
- Supported photo or document transfer through Siri still requires file confirmation and does not satisfy the low-friction intake requirement or replace COC document routing.
- A Shortcuts `Use Model` action may be considered only when the entire route is silent, bounded, subordinate to COC, and demonstrably useful. Its output may feed predetermined actions but may not become an independent task orchestrator.
- Keep the ChatGPT extension model out of scope unless a tested Shortcut proves a distinct benefit that direct COC or Codex routing cannot provide with equal or lower friction.
...
- No Apple Intelligence feature may independently determine material status, suppress required alerts, create canonical output, or be enabled before its use case, privacy route, failure behavior, COC visibility, and net ADHD workflow value have been evaluated.
...
- Observed: the processor path mismatch was corrected to `iCloud Drive / NEURO-DIV Intake / Pending`; five queued originals were archived and five raw-fallback COC records were read back as captured.
...
- Pending: make one recording from the physical iPhone Action Button, speak a short test phrase, tap to finish, and verify that the resulting file contains usable audio and continues through downstream COC processing.
