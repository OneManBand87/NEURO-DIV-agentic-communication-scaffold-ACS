...
- Device wiring status: the iPhone exposes both `Transcribe Audio` and `Use Model > On-Device`. The final magic-variable connection from recorded audio into transcription is not yet installed because iPhone Mirroring did not expose the required long-press variable selector. The incomplete actions were removed, leaving the verified raw-audio Action Button flow intact. The Mac is now unlocked, but Mirroring reports `iPhone in Use`; the one remaining user dependency is to lock the iPhone and leave it idle for Mirroring to reconnect. This corrected blocker was mirrored and read back through production CCS item `intake-2dfc3081-f790-4363-b241-224dac6741da`.
