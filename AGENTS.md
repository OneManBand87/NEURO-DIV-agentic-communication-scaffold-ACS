...
- Mandatory Codex routing through CCS: the Central Command Center (CCS), implemented by the ACS Command Center, is the orchestration and visibility layer for every NEURO-DIV workflow in which Codex participates. Codex-initiated or Codex-assisted requests, actions, device-control paths, approvals, state changes, blockers, outputs, and completion evidence must be created, routed, or mirrored through CCS. ChatGPT Remote, iPhone Mirroring, Shortcuts, mobile device management, connectors, and local tools are subordinate execution transports beneath CCS, not alternative top-level workflows. If CCS lacks a required capability, record the need and current state in CCS and preserve CCS visibility rather than silently bypassing it.
