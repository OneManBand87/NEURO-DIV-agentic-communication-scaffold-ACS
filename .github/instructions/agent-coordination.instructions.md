...
Codex and deterministic connector infrastructure own COC signal intake, deduplication, reconciliation, calculated aging, connector-health tracking, and action-candidate routing. Claude is reserved for explicitly assigned high-value coding and deep design work and must not run recurring monitoring, broad connector sweeps, routine COC orchestration, or multi-destination status publication. Treat the retired monitor's outputs as historical provenance only and leave `ACS-ACT-002` unexecuted until canonical reconciliation.
